...
const AppliedJobCard = ({ appliedJob, onWithdraw }: AppliedJobCardProps) => {
  ...
  const formattedDate = appliedJob.applicationDate
  ...
  return (
    ...
    >
        ...
        <Col xs={24} sm={6} className="flex flex-col items-end justify-between">
          ...
          <div className="flex w-full flex-col gap-2 sm:w-auto">
            ...
            {(appliedJob.status === "Accepted" ||
              appliedJob.status === "Completed") && (
              <Button
                icon={<StarOutlined />}
                onClick={() => setIsRatingModalVisible(true)}
                block
                className="border-yellow-500 text-yellow-600 hover:!text-yellow-700 hover:!border-yellow-600"
              >
                Đánh giá
              </Button>
            )}
      ...
      <RatingModal
        ...
        onSuccess={() => setIsRatingModalVisible(false)}
        rateeId={appliedJob.employerId}
        submissionId={appliedJob.candidateListId}
        rateeName={appliedJob.employerName}
      />
    </Card>
  );
};
