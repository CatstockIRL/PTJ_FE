...
const LocationEditField: React.FC<LocationEditFieldProps> = ({
  ...
  // Khởi tạo địa chỉ cụ thể từ fullLocation
  useEffect(() => {
    if (fullLocation) {
  ...
  // Load danh sách tỉnh/thành khi bắt đầu chỉnh sửa
  useEffect(() => {
  ...
  // Load danh sách quận/huyện khi tỉnh thay đổi
  useEffect(() => {
    if (selectedProvince) {
  ...
  // Load danh sách phường/xã khi quận thay đổi
  useEffect(() => {
    if (selectedDistrict) {
